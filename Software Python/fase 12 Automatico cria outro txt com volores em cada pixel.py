...
class Automatico():
    def auto(self, partid, orientacao, img, sensor, anda):
        y,x = partid
        partida = partid     
        saida = 0
        ...
        while saida < 1:
            ...
            for i in range(len(img.listaPosicoes)):
                if img.listaPosicoes[i] == partida and i != tam and i != (tam -1): #Se posição for iguar a já visitada e não é a ultima ou penultima
                    saida = saida + 1
                    img.mapeado = img.data
                    img.ultimaPosicao = partida
...
if __name__ == "__main__":
    ...
    while comando != 'SAIR':    
        if comando == 'MAPEADO':
            print img.mapeado
            img.salvaMapeado('mapeado/Mapa Mapeado.txt', img.mapeado, img.ultimaPosicao)
            
        comando = raw_input('Digite "F" frente, "D" 90 direita "E" 90 esquerda, "R" re, "A" automatico ou "sair": ')
        comando = comando.upper()
        if comando != 'SAIR':
            if comando == 'A':
                comando = auto.auto(partida, orientacao, img, sensor, anda)
            else:
